...
/**
 * Send coupon email to team participants using Brevo API
...
export async function sendCouponEmail({ teamName, couponId, qrCodeDataUrl, participants, teamSize }) {
  ...
  `;
    ...
    sendSmtpEmail.subject = `Your Conference Food Coupon - ${config.conference.name}`;
    sendSmtpEmail.htmlContent = emailHtml;
    sendSmtpEmail.sender = {
        name: "CSIT Department",
        email: config.email.from.match(/<(.+)>/)?.[1] || config.email.from
    };
    sendSmtpEmail.to = participantEmails.map(email => ({ email }));
    sendSmtpEmail.attachment = [{
        content: base64Data,
        name: `qrcode-${couponId}.png`
    }];

    try {
        const result = await apiInstance.sendTransacEmail(sendSmtpEmail);
        console.log(`✅ Email sent via Brevo API to: ${participantEmails.join(', ')}`);
